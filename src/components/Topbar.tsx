...
const TITLES: Record<string, [string, string]> = {
  ...
  summaryReport: ['Summary Report (TPL3)', 'รายงานสรุปวงเงินและการส่งบิลสะสม'],
  ...
  return (
    <header className="topbar">
      <div className="tb-l">
        ...
        <div>
          <div className="tb-title" style={{ lineHeight: '13px', fontFamily: 'Arial', fontSize: '12px' }}>{t}</div>
      ...
      <div className="tb-r" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        {(currentUser?.role === 'Administrator' || currentUser?.role === 'Executive') ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f8fafc', border: '1px solid #e2e8f0', padding: '4px 12px', borderRadius: '30px' }}>
            ...
            <span className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black text-white shrink-0" style={{ backgroundColor: (currentUser?.role === 'Administrator' || currentUser?.role === 'Executive') ? '#ef4444' : currentUser?.role === 'Accounting' ? '#4f46e5' : '#6b7280' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f8fafc', border: '1px solid #e2e8f0', padding: '4px 12px', borderRadius: '30px' }}>
            <span className="text-xs font-bold text-slate-700">
              {currentUser?.name} ({currentUser?.role || 'Employee'})
            </span>
            <span className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black text-white shrink-0" style={{ backgroundColor: '#6b7280' }}>
              {currentUser?.name ? currentUser.name.substring(0, 2).toUpperCase() : 'US'}
            </span>
          </div>
        )}
